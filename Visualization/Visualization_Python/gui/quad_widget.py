...
from utils.constants import VIEW_LOGS, FORBIDDEN_CURSOR, POINTING_CURSOR, GREEN, ARROW_CURSOR, TID, PACKET, BLACK, \
    LIGHTGRAY, COMPONENT_LOGS
from utils.type_names import QUAD, HBM
...
class QuadWidget(QWidget):
    ...
    def show_hbm_log_messages(self) -> None:
        try:
            dialog = LogColorDialog(self.quad.hbm, COMPONENT_LOGS.format(component=HBM), self)
            dialog.exec_()
        except Exception as e:
    ...
    def show_log_messages(self) -> None:
        ...
        try:
            dialog = LogColorDialog(self.quad, COMPONENT_LOGS.format(component=QUAD), self)
            dialog.exec_()
        except Exception as e:
